...
% Update costmap where beads are:
...
goalpt = bead.center(gotobead,:) - d*[sin(pushTh),cos(pushTh)];
TrajNum = 1;
[rowp{1},colp{1}] = runDijkstraPlanner(startpt,goalpt,map.cost,'false');

hold on
title('Trajectory to get first bead')
htraj = plot(colp{1},rowp{1},'.b');


hObstacle = subplot(2,2,4);
h(4) = imagesc(map.cost,[1 100]);
colormap(gray)
axis image
title('Cost Map')
...
%% Update bead plot
bead = updatePushedBead(state,bead,1);
...
switch mode
    ...
    case 1
        ...
        % Cycle to each bead and push bead right:
        for beadIdx = 1:nbead
            
        end
        
        % Plan a TrajLength Path
